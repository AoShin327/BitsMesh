        ...
        if (!empty($rule['factory'])) {
            ...
            if ($function->getNumberOfParameters() > 0) {
                ...
                $factory = function ($args) use ($callback, $callbackArgs) {
                    // PHP 8 compatibility: use positional args instead of named args
                    return call_user_func_array($callback, array_values($this->resolveArgs($callbackArgs, $args)));
                };
            } else {
                $factory = $callback;
            }
            ...
            $factory = function ($args) use ($factory, $calls) {
                ...
                foreach ($calls as $call) {
                    ...
                    call_user_func_array(
                        [$instance, $call[0]],
                        array_values($this->resolveArgs($call[1], [], $instance))
                    );
                }
    ...
    private function createSharedInstance($nid, array $rule, array $args) {
        if (!empty($rule['factory'])) {
            ...
            if ($function->getNumberOfParameters() > 0) {
                ...
                // PHP 8 compatibility: use positional args instead of named args
                $this->instances[$nid] = $instance = call_user_func_array($callback, array_values($callbackArgs));
            } else {
            ...
            if ($constructor && $constructor->getNumberOfParameters() > 0) {
                ...
                // PHP 8 compatibility: use positional args instead of named args
                $constructor->invokeArgs($instance, array_values($constructorArgs));
            } else {
        ...
        if (isset($class) && !empty($rule['calls'])) {
            foreach ($rule['calls'] as $call) {
                ...
                // PHP 8 compatibility: use positional args instead of named args
                $method->invokeArgs($instance, array_values($args));
            }
        }
